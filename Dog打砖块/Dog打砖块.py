...
for i in range(0,20):
    for j in range(0,10):
        ...
        if random.randint(0,3) == 1:
            ...
            brick_color.append((random.randint(100,200),random.randint(100,200),random.randint(100,200)))
...
while True:
    ...
    while not pause:
        ...
        tip3surf, tip3rect = f1.render("<按数字键1-5调速(默认为2)>", fgcolor=White, size=20)
        screen.blit(tip1surf, (0, 640))
        screen.blit(tip2surf, (0, 660))
        screen.blit(tip3surf, (0, 680))
        # 砖、球、板的绘制
        for brick in brick_list:
            pygame.draw.rect(screen, brick_color[brick_list.index(brick)], brick)
